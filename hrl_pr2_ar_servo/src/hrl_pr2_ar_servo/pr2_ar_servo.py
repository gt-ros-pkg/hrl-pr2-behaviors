...
class PR2ARServo(object):
    ...
    def servo_to_tag(self, pose_goal, goal_error=[0.03, 0.03, 0.1], initial_ar_pose=None):
        ...
        pid_x = PIDController(k_p=0.75, rate=rate, saturation=0.06)
        pid_y = PIDController(k_p=0.75, rate=rate, saturation=0.06)
        pid_r = PIDController(k_p=0.75, rate=rate, saturation=0.18)
        r = rospy.Rate(rate)
        self.preempt_requested = False
        while True:
            if rospy.is_shutdown():
